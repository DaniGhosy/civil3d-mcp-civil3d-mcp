...
/// <summary>
/// Label style listing/creation for civil3d_label (list_label_styles, list_labels, add_label).
/// Distinct from LabelCommands.cs (which extracts DBText/MText/Leader/Dimension entities read
/// from the drawing) — this file manages Civil 3D's own label/label-set OBJECTS attached to
/// alignments/profiles/surfaces/pipes/structures. Label object creation types
/// (SurfaceSpotElevationLabel, ProfileStationLabel, etc.) are resolved via reflection since the
/// exact managed type name/namespace shifts between Civil 3D releases and isn't worth hard-coding
/// — Civil3DCompatibility.FindLoadedType/TryInvokeStaticOverloads try each candidate name and each
/// Create() overload until one accepts the built arguments.
/// </summary>
public static class LabelStyleCommands
{
  ...
  public static Task<object?> AddLabelAsync(JsonObject? parameters)
  {
    ...
    return CivilExecution.WriteAsync<object?>((doc, civilDoc, database, transaction) =>
    {
      ...
      {
        var pointNode = parameters?["point"] as JsonObject
