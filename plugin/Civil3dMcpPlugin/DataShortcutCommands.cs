...
public static class DataShortcutCommands
{
  ...
  // -------------------------------------------------------------------------
  // listDataShortcuts / createDataShortcut / referenceDataShortcut /
  // syncDataShortcuts / promoteDataShortcut (ported from source): a distinct,
  // higher-level data-shortcut inventory/publish/reference/sync workflow from
  // the DataShortcuts static class above. Publish/reference/sync launch the
  // real Civil 3D commands (SendStringToExecute) that open their native
  // dialogs — Civil 3D still requires user confirmation there, so responses
  // report status "initiated"/"manual_step_required" rather than claiming
  // full automation.
  // -------------------------------------------------------------------------
  ...
  {
    if (parameters?[name] is not JsonArray array)
    {
