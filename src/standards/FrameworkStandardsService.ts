...
/**
 * civil3d_framework_rules.json holds 29 rules extracted from a Civil 3D CAD-standards
 * framework PDF. The original automated extraction interleaved fragments from adjacent
 * bullet points and headings (classic PDF column-bleed). The title/rule text here has
 * been cleaned up and rewritten into complete, coherent sentences based on the extracted
 * fragments plus general Civil 3D CAD-management knowledge (these are recognizable,
 * standard practices: never edit a style in a production drawing, use Copy/New before
 * changing a style, purge Regapps with -PURGE, the "(An)" prefix for annotative styles,
 * etc.). section_number/pages/tags/score are structured metadata from the extraction and
 * were left as-is. This is reconstructed content, not a literal transcription of the
 * source PDF — treat it as a paraphrase of real guidance, not a verbatim quote.
 */
...
function getPromptRulesPath(): string {
  ...
  return path.resolve(currentDirectory, "./data/civil3d_framework_rules.json");
}
