...
export function renderVideoPlayerHtml(video: HelpVideoRef): string {
  ...
  const webmSource = video.webmUrl ? `<source src="${escapeHtml(video.webmUrl)}" type="video/webm">` : "";
  ...
  try {
    ...
    for (const result of results) {
      ...
      for (const page of result.video_pages ?? []) {
        ...
        const mp4Url = (page.video_urls ?? []).map((url) => trustedAutodeskUrl(url, ".mp4")).find(Boolean);
        if (!pageUrl || !mp4Url) continue;
        const webmUrl = (page.video_urls ?? []).map((url) => trustedAutodeskUrl(url, ".webm")).find(Boolean);
